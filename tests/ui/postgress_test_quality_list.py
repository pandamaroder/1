...
@pytest.fixture
def get_citylist_from_postgres():
    ...
    # выбор города из БД
    organization_select = "SELECT name FROM service.branch LIMIT 10"
    ...
    if cur.rowcount > 0:
        ...
        organization_input = organization_name[0:18].lower()
        #organization_input = organization_name[0:49].lower()
    ...
    hint_select = f"SELECT name FROM service.branch WHERE name ILIKE '%{organization_input}%'"
    cur.execute(hint_select)
    for row in cur:
        hint_list.append(row)
    conn.close()
...
class TestQualityPage:
    ...
    def test_qualityy(self, get_citylist_from_postgres):
        """
        ...
        page.search_organizations(get_citylist_from_postgres[0], get_citylist_from_postgres[1])